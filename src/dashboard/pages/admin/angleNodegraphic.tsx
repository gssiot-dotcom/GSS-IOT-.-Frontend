...
const SensorGraph: React.FC<SensorGraphProps> = ({
         ...
         try {
            ...
            const mergedData = graphData.map(d => {
               const closestTime = Object.keys(windDataMap).reduce((prev, curr) => {
                  return Math.abs(Number(curr.replace(':','')) - Number(d.time.replace(':',''))) <
                         Math.abs(Number(prev.replace(':','')) - Number(d.time.replace(':','')))
                     ? curr
                     : prev
               })
   ...
   const getYAxisTicks = () => {
      ...
      for (let v = lower; v <= upper; v += step) ticks.push(parseFloat(v.toFixed(1)))
      return ticks
   }
               ...
               <div className='flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2'>
                  ...
                  <div className='flex flex-row items-center justify-between sm:justify-end gap-3'>
                     <div className='flex items-center gap-x-2'>
                        <label htmlFor='time-filter' className='text-xs sm:text-sm font-medium text-gray-700 whitespace-nowrap'>
                           기간:
                        </label>
                        <Select value={hours.toString()} onValueChange={v => onSelectTime(Number(v))}>
                           ...
                           <SelectContent>
                              <SelectItem value='1' className='text-xs sm:text-sm'>1 시간</SelectItem>
                              <SelectItem value='6' className='text-xs sm:text-sm'>6 시간</SelectItem>
                              <SelectItem value='12' className='text-xs sm:text-sm'>12 시간</SelectItem>
                              <SelectItem value='24' className='text-xs sm:text-sm'>24 시간</SelectItem>
                           </SelectContent>
                        </Select>
                     </div>
               ...
               <div className='w-full h-full pb-4 px-1 sm:px-2'>
                  <ResponsiveContainer width='104%' height={330}>
                     <LineChart key={data.length} data={data} margin={getChartMargins()}>
                        ...
                        {!isMobile && (
                           <Legend wrapperStyle={{ fontSize: '12px',marginBottom: '-10px', paddingTop: '10px' }} />
                        )}
