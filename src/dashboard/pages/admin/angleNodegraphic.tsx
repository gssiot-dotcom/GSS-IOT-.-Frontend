...
const WeekInput = forwardRef<HTMLInputElement, WeekInputProps>(
    ({ value, onClick, ...rest }, ref) => {
        ...
        return <input ref={ref} onClick={onClick} value={display} readOnly {...rest} className="border border-slate-400 rounded px-2 py-1 text-xs" />
    }
)
...
const SensorGraph: React.FC<SensorGraphProps> = ({
    ...
    return (
        <div className='ml-auto h-full w-full sm:w-[95%] md:w-[85%] lg:w-[69.4%] 2xl:w-[68.8%] 3xl:w-[69%] pb-5 md:-mr-2 2xl:-mr-5 3xl:-mr-1 2xl:h-[20%]'>
            <Card className='w-full border shadow-sm border-slate-400 mt-4 sm:mt-6'>
                        ...
                        <div className="flex flex-row items-center justify-between sm:justify-end gap-3">
                            <div className="flex items-center gap-x-2">
                                ...
                                <Select value={timeMode} onValueChange={v => setTimeMode(v as any)}>
                                    <SelectTrigger className="h-6 w-[90px] sm:w-[120px] text-xs md:text-sm border border-slate-400"><SelectValue placeholder="Select mode" /></SelectTrigger>
                                    <SelectContent>
                                ...
                                {timeMode === "hour" && <Select value={hours.toString()} onValueChange={v => onSelectTime(Number(v))}><SelectTrigger className="h-6 w-[90px] sm:w-[120px] text-xs md:text-sm border border-slate-400"><SelectValue placeholder="Select time" /></SelectTrigger><SelectContent><SelectItem value="1">1 시간</SelectItem><SelectItem value="6">6 시간</SelectItem><SelectItem value="12">12 시간</SelectItem><SelectItem value="24">24 시간</SelectItem></SelectContent></Select>}
                                {timeMode === "day" && <DatePicker selected={selectedDate} onChange={(date) => setSelectedDate(date)} locale={ko} dateFormat="yyyy-MM-dd" className="border border-slate-400 rounded px-2 py-1 text-xs" placeholderText="날짜 선택" popperClassName="z-[9999]" portalId="root" />}
                                {timeMode === "week" && (<DatePicker selected={selectedDate} onChange={(date) => setSelectedDate(date)} locale={ko} customInput={<WeekInput />} placeholderText="주차 선택" popperClassName="z-[9999]" portalId="root" filterDate={(date) => date.getDay() === 1} />)}
                                {timeMode === "month" && <DatePicker selected={selectedDate} onChange={(date) => setSelectedDate(date)} locale={ko} showMonthYearPicker dateFormat="yyyy-MM" className="border border-slate-400 rounded px-2 py-1 text-xs" placeholderText="월 선택" popperClassName="z-[9999]" portalId="root" />}
                            </div>
                            <Badge variant="outline" className="h-6 text-xs md:text-sm border-slate-400">데이터 수: {data.length}</Badge>
                        </div>
                    </div>
                ...
                <CardContent className='p-0 pt-2 overflow-x-hidden overflow-visible' ref={containerRef}>
                    <div className='w-full h-[280px] sm:h-[320px] md:h-[350px] lg:h-[38.9vh] 2xl:h-[46.5vh] 3xl:h-[41.3vh] px-1 sm:px-2 '>
                        <ResponsiveContainer
