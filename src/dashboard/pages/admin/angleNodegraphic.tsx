...
import {
	...
	GraphDataPoint,
	TopNodeGraphPoint,
} from '@/dashboard/pages/admin/AngleNodes'
...
const SensorGraph: React.FC<SensorGraphProps> = ({
	...
	const [data, setData] = useState<
		GraphDataPoint[] | TopNodeGraphPoint[] | DeltaGraphPoint[]
	>(graphData)
	...
	const getYDomainAndTicks = (
		data: GraphDataPoint[] | TopNodeGraphPoint[] | DeltaGraphPoint[]
	) => {
		...
		if (viewMode === 'delta') {
			...
			if (values.length === 0)
				return { domain: [-0.01, 0.01], ticks: [-0.01, 0, 0.01] }
