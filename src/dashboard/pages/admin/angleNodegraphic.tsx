...
function MobileTooltip({
    ...
    const timeStr =
        timeMode === 'week' || timeMode === 'month' ? `${yyyy}-${MM}-${dd}` : `${yyyy}-${MM}-${dd} ${hh}:${mm}`
            ...
            <div className="space-y-0.5">
                {items.map((it: any, idx: number) => (
                    <div key={idx} className="flex justify-between gap-3">
                        <span className="text-gray-600 truncate max-w-[140px]">{it.name}</span>
                        <span className="text-gray-900 font-medium">{Number.isFinite(it.value) ? it.value.toFixed(2) : '-'}</span>
                    </div>
                ))}
            </div>
        </div>
...
const SensorGraph: React.FC<SensorGraphProps> = ({
    ...
    const attachWindData = (points: any[]) => {
        ...
        if (!windHistory || windHistory.length === 0) {
            return points.map(p => ({ ...p, timestamp: new Date(p.time).getTime(), wind_speed: null }))
        }
        ...
        return points.map(p => {
            ...
            return { ...p, timestamp: sensorTimestamp, wind_speed: sortedWindHistory[closestIndex]?.wind_speed ?? null }
        })
    }
    ...
    const getYDomainAndTicks = (d: any[]) => {
        ...
        if (viewMode === 'general' || viewMode === 'top6') {
            // general/top6: angle 축 기준
            const values = d
                .flatMap(x => [(x as any).angle_x, (x as any).angle_y, ...(topDoorNums ?? []).map(n => (x as any)[`node_${n}`])])
            ...
            <div className="flex flex-wrap gap-1 px-3 pb-2">
                ...
                {viewMode === 'top6' &&
                    (topDoorNums ?? []).slice(0, 6).map(n => (
                        <Badge key={n} variant="outline" className="text-[11px] border-slate-300">
                            N-{n}
                        </Badge>
                    ))}
            </div>
