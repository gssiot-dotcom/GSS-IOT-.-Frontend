...
import { IAngleNode, IBuilding, SensorData, GraphDataPoint, DeltaGraphPoint } from '../../../types/interfaces'
...
const AngleNodes = () => {
      ...
      .then(res => {
        ...
        if (viewMode === 'general') {
          ...
          for (let i = 1; i < sortedData.length; i++) {
            const time = new Date(sortedData[i].createdAt).toLocaleTimeString('ko-KR', {
              hour: '2-digit',
              minute: '2-digit',
              hour12: false,
            })
            deltaGraphData.push({ time, [`node_${selectedDoorNum}`]: sortedData[i].angle_x - sortedData[i - 1].angle_x })
          }
  ...
  const handleSetAlarmLevels = async (levels: { B: number; G: number; Y: number; R: number }) => {
    if (!buildingId) return
    try {
