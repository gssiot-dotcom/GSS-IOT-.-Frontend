...
function pickCalibratedXY(d: any) {
	const cx =
		d?.calibrated_x ?? d?.calibratedX ?? d?.angle_x ?? d?.angleX ?? null
	const cy =
		d?.calibrated_y ?? d?.calibratedY ?? d?.angle_y ?? d?.angleY ?? null
...
const AngleNodes = () => {
	...
	const top6GraphData = useMemo(() => {
		...
		topDoorNums.forEach((dn, idx) => {
			...
			arr.forEach(item => {
				...
				if (!map.has(ts))
					map.set(ts, { time: new Date(ts).toISOString(), timestamp: ts })
	...
	}, [queryClient, refetchGraph, refetchWind])

	const handleAngleRealtime = useCallback(
		(newData: SensorData) => {
			console.log('Angle node-socket data', newData)
			if (!buildingId) return
			...
			// 2) 그래프(+wind)
			if (viewMode !== 'top6') {
				if (selectedDoorNum != null && newData.doorNum === selectedDoorNum) {
					scheduleRefetch()
				}
				return
			}
			...
			if (topDoorNums?.includes(newData.doorNum)) {
				scheduleRefetch()
			}
		},
		[
