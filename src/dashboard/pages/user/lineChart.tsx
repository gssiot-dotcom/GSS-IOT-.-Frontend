...
export function ChartByDateFilters() {
	...
	return (
		<div className='w-full h-full grid grid-cols-1 md:flex flex-col gap-y-5 md:text-xl text-lg'>
			<Card className='w-full h-fit border border-gray-400'>
				<CardHeader className='flex items-center gap-2 space-y-0 border-b border-gray-400 py-5 sm:flex-row'>
					<div className='grid flex-1 gap-1 text-center sm:text-left'>
						<CardTitle>인터랙티브 영역 차트</CardTitle>
						<CardDescription>
							최근 3개월 동안 가장 많이 사용된 노드들.
						</CardDescription>
					</div>
				...
				<CardContent className='px-2 pt-4 sm:px-6 sm:pt-6'>
					...
					>
						<AreaChart data={filteredData}>
							...
							<XAxis
								dataKey='date'
								tickLine={{ stroke: '#AAB0B5' }} // oy kunlarning ustidagi kichik chiziq
								tickMargin={8}
								minTickGap={32}
								axisLine={{ stroke: '#AAB0B5' }} // Asosiy chiziq rangi chart line ning neg pastidagi
								tickFormatter={value => {
