...
const AngleNodeScroll = ({
   ...
   return (
      <div className="flex flex-col"> {/* 부모 컨테이너에 flex-col 추가 */}
         {/* 필터링된 노드 및 위험 노드 섹션 */}
               ...
               <div className="grid grid-cols-2 md:gap-6 gap-2 md:p-5 p-2">
                  {sortedNodes.map((item) => (
                     <Card
                        ...
                        className={`border border-slate-300 flex flex-col justify-center p-4
                                    ${getNodeColorClass(item)}
                                    shadow-md hover:shadow-lg transition duration-200 ease-in-out rounded-xl cursor-pointer`}
         ...
         <div className="flex flex-col items-center z-50  p-2 mt-[-30vh] mr-[20vw] ml-[27vw] mb-[2.9vh] order-2"> {/* order-2로 이전 섹션 아래에 표시 */}
            <img
               src={digaenode}
               alt="디가에 노드 이미지"
