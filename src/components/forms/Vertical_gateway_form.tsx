...
const pickDoorNum = (x: any) => {
	const v = x?.doorNum ?? x?.door_num ?? x?.node_number ?? x?.nodeNum ?? x?.node
	const n = Number(v)
...
const isUsable = (x: any) => {
	const v =
		x?.status ?? x?.node_status ?? x?.nodeStatus ?? x?.active ?? x?.isActive
...
const VerticalGatewayForm = ({
	refetchNodes,
	gateways,
}: VerticalGatewayFormProps) => {
	const [checkedNodeNumbers, setCheckedNodeNumbers] = useState<number[] | null>(
		null,
	)
				...
				>
					...
					<div className='w-full'>
						...
						<div className='mt-2 flex items-center gap-3'>
							...
							{missingMsg && (
								<span className='text-sm text-red-500'>{missingMsg}</span>
							)}
						</div>
					</div>
					...
					<div className='mb-2 text-sm'>
						<label className='flex items-center gap-x-2 text-gray-700'>
							노드 선택:
							{checkedNodeNumbers && (
								<span>{checkedNodeNumbers.length} 개 노드 선택함</span>
							)}
						</label>
